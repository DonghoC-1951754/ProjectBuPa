...
print(trace)
```
#kijken naar de throughput en dan zien waar die niet gelijk is aan 0 voor de moeilijke
```{r}
ond3_2 <- ond3 %>%
  group_by(casePurchasingDocument) %>%
  arrange(eventtimetimestamp) %>%
  mutate(ThroughputTime = as.numeric(difftime(lead(eventtimetimestamp), eventtimetimestamp, units = "hours")))
