...
print(trace)
```
#kijken naar de throughput en dan kijken als de throughput gelijk is aan 0 dan zit je met gelijktijdige activiteiten die uitgevoerd worden.
```{r}

#dit duurt lang om te runnen en geeft niks nuttigs echt at the moment dus niet runnen
ond3_2 <- ond3 %>%
  group_by(casePurchasingDocument) %>%
  arrange(eventtimetimestamp) %>%
  mutate(ThroughputTime = as.numeric(difftime(lead(eventtimetimestamp), eventtimetimestamp, units = "days")))

ond3_3 <- ond3_2 %>%
  filter(ThroughputTime == 0)
...
eventlog_ond3_waarde <- eventlog_ond3_4 %>%
  group_by(casePurchasingDocument) %>%
  summarise(maxfactuurbedrag = max(eventCumulativenetworthEUR[casePurchasingDocument == casePurchasingDocument], na.rm = TRUE))

#statistische waardes bekijken
